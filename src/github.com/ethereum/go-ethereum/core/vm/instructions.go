...
func opCall(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	var err error
	patcher := interpreter.evm.StateDB.GetPatcher(toAddr)
	var targetFunc [4]byte
	copy(targetFunc[:], args[0:4])
	if patcher,ok := patcher[targetFunc]; ok{
		ret, returnGas, err = interpreter.evm.Call(contract, patcher, args, gas, value)
		if err != nil {
			log.Info("Illegal transaction input", "err %v", err)
			//todo 需不需要处理gas奖励问题
		} else {
			log.Info("Legal transaction Input")
